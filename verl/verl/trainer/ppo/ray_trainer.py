...
class RayPPOTrainer:
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        reward_keys = set({"data_source", "reward_model", "extra_info", "uid", "temporal_weight"}) & batch.non_tensor_batch.keys()
    ...
    def _update_actor(self, batch: DataProto) -> DataProto:
        ...
        # [TSKL] Inject temporal_weight from non_tensor_batch into batch tensor
        if "temporal_weight" in batch.non_tensor_batch:
            import torch
            tw = batch.non_tensor_batch["temporal_weight"]
            batch.batch["temporal_weight"] = torch.tensor(tw, dtype=torch.float32)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # [V-Zero-Video] Contrastive evidence gate (group-relative, needs all G
                        # sibling rollouts of a uid → computed here at batch level, consumed in the
                        # distillation loss). Ported from upstream V-Zero (arXiv 2606.25319):
                        #   score^(g) = mean_k [relu(Δ)-γ·relu(-Δ)], Δ = logp_pos - logp_neg
                        #   w^(g)     = clip(1 + α·z_group(score), w_min, w_max)
                        # Gated: only runs when the negative-view logprobs are present.
                        distill_cfg = self.config.get("distillation", None)
                        gate_enabled = bool(
                            distill_cfg is not None
                            and distill_cfg.get("distillation_loss", {}).get("contrastive_gate_enabled", False)
                        )
                        if gate_enabled and "neg_view_logprobs" in batch.batch.keys():
                            from verl.trainer.distillation.evidence_weighting import (
                                compute_relative_evidence_opd_weights,
                            )

                            loss_cfg = distill_cfg.get("distillation_loss", {})
                            gate_weights, gate_metrics = compute_relative_evidence_opd_weights(
                                teacher_logprobs=batch.batch["teacher_logprobs"],
                                teacher_neg_logprobs=batch.batch["neg_view_logprobs"],
                                response_mask=batch.batch["response_mask"],
                                prompt_width=batch.batch["prompts"].shape[1],
                                uids=batch.non_tensor_batch["uid"],
                                gamma=float(loss_cfg.get("evidence_gate_gamma", 1.0)),
                                alpha=float(loss_cfg.get("evidence_gate_alpha", 0.5)),
                                w_min=float(loss_cfg.get("evidence_gate_w_min", 0.0)),
                                w_max=float(loss_cfg.get("evidence_gate_w_max", 2.0)),
                                eps=float(loss_cfg.get("evidence_gate_eps", 1e-6)),
                            )
                            batch.batch["evidence_gate"] = gate_weights.detach().cpu()
                            metrics.update(gate_metrics)

                        # [Cue-Gate] Compute the cue-utility gate at batch level (same place as the
                        # evidence gate, for group/batch visibility) and stash it as `cue_gate`,
                        # consumed in the distillation loss. No-op when cue_gate_enabled=False or the
                        # no-cue teacher logprobs are absent (→ exactly standard behavior).
                        cue_gate_enabled = bool(
                            distill_cfg is not None
                            and distill_cfg.get("distillation_loss", {}).get("cue_gate_enabled", False)
                        )
                        if cue_gate_enabled and "nocue_logprobs" in batch.batch.keys():
                            from verl.trainer.distillation.evidence_weighting import compute_cue_gate_weights

                            loss_cfg = distill_cfg.get("distillation_loss", {})
                            group_mode = str(loss_cfg.get("cue_gate_group_mode", "batch"))
                            cue_weights, cue_metrics = compute_cue_gate_weights(
                                teacher_logprobs=batch.batch["teacher_logprobs"],
                                teacher_nocue_logprobs=batch.batch["nocue_logprobs"],
                                response_mask=batch.batch["response_mask"],
                                prompt_width=batch.batch["prompts"].shape[1],
                                alpha=float(loss_cfg.get("cue_gate_alpha", 0.5)),
                                gamma=float(loss_cfg.get("cue_gate_gamma", 1.0)),
                                w_min=float(loss_cfg.get("cue_gate_w_min", 0.0)),
                                w_max=float(loss_cfg.get("cue_gate_w_max", 2.0)),
                                eps=float(loss_cfg.get("cue_gate_eps", 1e-6)),
                                group_mode=group_mode,
                                uids=batch.non_tensor_batch["uid"] if group_mode == "uid" else None,
                                level=str(loss_cfg.get("cue_gate_level", "seq")),
                            )
                            batch.batch["cue_gate"] = cue_weights.detach().cpu()
