...
def left_right_2_no_padding(data: TensorDict) -> TensorDict:
    ...
    # [ExOPD] (bsz, seqlen, 1) — base model logprobs
    base_logprobs = data.get("base_logprobs", None)
    if base_logprobs is not None:
        base_logprobs_rmpad = index_first_axis(base_logprobs.unsqueeze(-1).flatten(0, 1), indices)
        base_logprobs_nested = torch.nested.nested_tensor_from_jagged(
            base_logprobs_rmpad.squeeze(-1), offsets=cu_seqlens
        )
        data["base_logprobs"] = base_logprobs_nested

    # [AD-ExOPD] (bsz, seqlen, 1) — dense-frame base model logprobs
    base_dense_logprobs = data.get("base_dense_logprobs", None)
    if base_dense_logprobs is not None:
        base_dense_logprobs_rmpad = index_first_axis(base_dense_logprobs.unsqueeze(-1).flatten(0, 1), indices)
        base_dense_logprobs_nested = torch.nested.nested_tensor_from_jagged(
            base_dense_logprobs_rmpad.squeeze(-1), offsets=cu_seqlens
        )
        data["base_dense_logprobs"] = base_dense_logprobs_nested
