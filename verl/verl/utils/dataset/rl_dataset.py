...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        # [Visual-Cue] optional column holding a teacher-only prompt (student prompt + cue hint).
        # When absent (default None) behavior is unchanged.
        self.teacher_prompt_key = config.get("teacher_prompt_key", None)
        # [AFD span-directed] optional column holding a per-sample evidence time window
        # (relative [s, e] in [0,1]) for span-directed teacher superset sampling. When absent
        # (default None) behavior is unchanged. Value may be None per-row → uniform superset.
        self.answer_span_key = config.get("answer_span_key", None)
    ...
    def __getitem__(self, item):
        ...
        # [Visual-Cue] build teacher-only prompt (student prompt + cue) BEFORE popping the
        # multimodal columns, since it also contains <video>/<image> placeholders. Gated on
        # teacher_prompt_key being set AND the column existing & non-null for this row.
        if (
            self.teacher_prompt_key is not None
            and self.teacher_prompt_key in row_dict
            and row_dict[self.teacher_prompt_key] is not None
        ):
            row_dict["teacher_raw_prompt"] = self._build_messages(row_dict, key=self.teacher_prompt_key)

        # [AFD span-directed] normalize the evidence-span column to a plain [s, e] float list
        # (parquet may store it as a numpy array) or None. Exposed under a fixed key the agent
        # loop reads. Gated: only when answer_span_key is configured. None → uniform superset.
        if self.answer_span_key is not None:
            sp = row_dict.get(self.answer_span_key, None)
            norm_span = None
            if sp is not None:
                try:
                    if len(sp) == 2:
                        norm_span = [float(sp[0]), float(sp[1])]
                except (TypeError, ValueError):
                    norm_span = None
            row_dict["answer_span"] = norm_span
    ...
    async def process_vision_info(
        ...
        try:
            ...
            logger.warning(f"Failed to process vision info, skipping this sample: {e}")
    ...
    def _process_multi_modal_info(
        ...
        if has_visual:
            ...
            # [AFD] Optional student frame budget: cap frames per video via config.
            # Backward-compatible — when `config.video_max_frames` is unset, messages are
            # untouched and qwen_vl_utils uses its defaults exactly as before. We only
            # inject when a video element does NOT already carry its own frame hint
            # (so the teacher's dense-frame injection is never overridden here).
            video_max_frames = config.get("video_max_frames", None) if config is not None else None
                ...
                for message in messages:
                    ...
                    for item in content:
                        if (
                            ...
                            and "max_frames" not in item
                            and "nframes" not in item
                        ):
                            item["max_frames"] = int(video_max_frames)

            # Optional video fps override: inject fps into video items so
            # qwen_vl_utils samples at this rate instead of the default 2.0.
            # Backward-compatible: unset → no injection, default FPS=2.0 applies.
            video_fps = config.get("video_fps", None) if config is not None else None
            ...
            try:
                ...
                logger.warning(f"Failed to decode video in _process_multi_modal_info, skipping: {e}")
                images, videos = None, None
        else:
            images, videos = None, None
