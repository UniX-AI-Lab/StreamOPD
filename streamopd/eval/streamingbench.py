"""
StreamingBench evaluation under the memory-free recent-window streaming protocol.

Only the recency baseline is supported (--top-k 0): decode the time window ending at each
question timestamp and answer from the most recent N chunks.
...
Usage:
    CUDA_VISIBLE_DEVICES=0 python -m streamopd.eval.streamingbench \
        ...
        --qa-model /path/to/checkpoint \
        --top-k 0 --recent-frames-only 4 --chunk-duration 1.0 --fps 1.0 \
        --output-dir results/streamingbench_recent4
"""
...
from streamopd.streaming.qwen3_5 import (
    RecentWindowQAModel,
    query_recent_window,
)
...
def print_summary(results: list[dict[str, Any]]) -> None:
    ...
    print("\n" + "=" * 60)
    print("StreamingBench Recent-Window Results (Qwen3.5-VL)")
    print("=" * 60)
...
def run_benchmark(
    ...
    dense_frames: int | None = None,
) -> None:
    if top_k != 0:
        raise ValueError(
            "This script only supports the no-retrieval baseline. "
    ...
    logger.info("Loading Qwen3.5-VL model: %s", qa_model)
    ...
    logger.info("Model loaded successfully. enable_thinking=%s", enable_thinking)
        ...
        for video_index, (video_path_raw, questions) in enumerate(video_questions.items(), start=1):
            ...
            for question in questions:
                ...
                if dense_frames is not None:
                    video_start = 0.0
                    effective_recent_chunks = int(dense_frames)
                else:
                ...
                try:
                    result, decode_backend = query_recent_window(
                        ...
                        nframes=int(dense_frames) if dense_frames is not None else None,
                    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="StreamingBench recent-window evaluation (Qwen3.5-VL)")
    parser.add_argument("--anno-path", required=True)
    parser.add_argument("--video-dir", required=True)
    parser.add_argument("--output-dir", default=None)
    parser.add_argument("--qa-model", required=True, help="HuggingFace-format checkpoint to evaluate")
    ...
    parser.add_argument("--dense-frames", type=int, default=None,
                        help="If set, use uniform sampling of N frames over [0, ts_sec] instead of recent-window.")
    parser.add_argument("--enable-thinking", action="store_true",
                        help="Use Qwen3.5 thinking-on prompt (appends '<think>\\n'). "
                             "Default off appends '<think>\\n\\n</think>\\n\\n'.")
    args = parser.parse_args()

    output_dir = args.output_dir or os.path.join(
        "results", f"streamingbench_recent{args.recent_frames_only}"
    )
    run_benchmark(
