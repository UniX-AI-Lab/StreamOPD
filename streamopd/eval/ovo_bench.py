"""
OVO-Bench evaluation under the memory-free recent-window streaming protocol.

Pipeline: decode video -> chunk by time -> keep the last N chunks -> generate_from_frames.

Usage:
    CUDA_VISIBLE_DEVICES=0 python -m streamopd.eval.ovo_bench \
        --model_path /path/to/checkpoint \
        --anno_path data/benchmarks/ovo_bench/ovo_bench_new.json \
        --chunked_dir data/benchmarks/ovo_bench/chunked_videos \
        --result_dir results/ovo_recent4 \
...
from streamopd.constants import BACKWARD_TASKS, FORWARD_TASKS, REAL_TIME_TASKS
from streamopd.streaming.recent_window import load_jsonl_results
...
def main() -> None:
    ...
    parser.add_argument("--dense_frames", type=int, default=None,
                        help="If set, use uniform sampling of N frames over the whole clip instead of recent-window.")
    parser.add_argument(
