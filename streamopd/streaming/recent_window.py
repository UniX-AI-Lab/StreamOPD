...
from streamopd.constants import (
    BACKWARD_TASKS,
    REAL_TIME_TASKS,
    FORWARD_TASKS,
    BR_PROMPT_TEMPLATE,
...
class RecentWindowQAModel:
    ...
    def _flatten_vision_features(self, features: Any) -> torch.Tensor:
        if isinstance(features, torch.Tensor):
            return features
        # Handle BaseModelOutputWithPooling / BaseModelOutputWithDeepstackFeatures
        ...
        if hasattr(features, "last_hidden_state"):
            lhs = features.last_hidden_state
...
def decode_video_to_chunks_qwen(
    ...
    if use_exact_recent:
        try:
            from streamopd.streaming.exact_recent_decoder import fetch_recent_video_exact
    ...
    if nframes is not None:
        video_req: dict[str, Any] = {"video": video_path, "nframes": int(nframes)}
        if use_exact_recent:
            raise ValueError("nframes mode is incompatible with QWEN_EXACT_RECENT_DECODE=1")
    else:
        video_req = {"video": video_path, "fps": float(fps)}
    ...
    if use_exact_recent:
        ...
        try:
            ...
            # qwen_vl_utils raises "nframes should in interval [X, Y], but got N" when
            # the clip is shorter than requested nframes. Clamp to Y (must be even per FRAME_FACTOR=2)
            # and retry once. Applies only to dense-N sampling mode.
            msg = str(exc)
            m = re.search(r"nframes should in interval \[(\d+), (\d+)\], but got \d+", msg)
            if nframes is not None and m is not None:
                lo, hi = int(m.group(1)), int(m.group(2))
                clamped = max(lo, (hi // 2) * 2)
                video_req["nframes"] = clamped
                video, metadata = fetch_video(video_req, return_video_metadata=True)
            else:
                raise
